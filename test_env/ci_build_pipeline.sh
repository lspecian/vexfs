...
# QEMU-based automated build pipeline for continuous integration
...
check_ci_dependencies() {
    ...
    local missing_deps=()
    local required_tools=("qemu-system-x86_64" "qemu-img" "cloud-localds" "git" "curl")
    ...
    # Check disk space (require at least 15GB)
...
run_prebuild_checks() {
    ...
    cd "$PROJECT_ROOT"
    if cargo check --lib --target=x86_64-unknown-linux-gnu > "$CI_LOG_DIR/rust-check.log" 2>&1; then
        log_success "Rust syntax check passed"
    else
        log_error "Rust syntax check failed. Check log: $CI_LOG_DIR/rust-check.log"
    fi
    
    # Check build script
    if [ ! -x "$SCRIPT_DIR/build_vexfs_image.sh" ]; then
        log_error "Build script not found or not executable: $SCRIPT_DIR/build_vexfs_image.sh"
    fi
...
build_ci_images() {
    ...
    for variant in "${BUILD_VARIANTS[@]}"; do
        ...
        local build_log="$CI_LOG_DIR/build-${variant}-${CI_BUILD_NUMBER}.log"
        
        # Build image using the build script
        local build_cmd=(
            "$SCRIPT_DIR/build_vexfs_image.sh"
            --variant "$variant"
        ...
        if "${build_cmd[@]}" > "$build_log" 2>&1; then
            log_success "$variant image build completed"
        else
            log_error "$variant image build failed. Check log: $build_log"
    ...
    log_success "All image builds completed in ${build_duration}s"
}
...
run_ci_validation() {
    ...
    local validation_cmd=(
        "$SCRIPT_DIR/validate_vexfs_image.sh"
...
qemu-system-x86_64 -m 2048 \\
  -drive file=vexfs-VARIANT-$VEXFS_VERSION.qcow2,format=qcow2 \\
...
<body>
    ...
    <table>
        ...
        <tr><td>Pre-build Checks</td><td class="success">✅ Passed</td><td>Rust syntax check completed</td></tr>
        <tr><td>Image Build</td><td class="success">✅ Passed</td><td>All variants built successfully</td></tr>
        <tr><td>Validation</td><td class="success">✅ Passed</td><td>All images validated</td></tr>
        <tr><td>Artifact Generation</td><td class="success">✅ Passed</td><td>Artifacts and checksums generated</td></tr>
    </table>

    <h2>Built Images</h2>
    <table>
        <tr><th>Variant</th><th>Image File</th><th>Size</th><th>Compressed Size</th></tr>
    ...
    <h2>Usage</h2>
    <pre><code>qemu-system-x86_64 -m 2048 \
  -drive file=vexfs-VARIANT-VERSION_PLACEHOLDER.qcow2,format=qcow2 \
  -netdev user,id=net0,hostfwd=tcp::2222-:22 \
  -device virtio-net,netdev=net0</code></pre>
</body>
</html>
EOF
    ...
    local images_table=""
    for variant in "${BUILD_VARIANTS[@]}"; do
        local image_file="$CI_OUTPUT_DIR/vexfs-${variant}-${VEXFS_VERSION}.qcow2"
        local compressed_file="$CI_OUTPUT_DIR/vexfs-${variant}-${VEXFS_VERSION}.qcow2.gz"
        ...
        if [ -f "$image_file" ]; then
            ...
            images_table+="<tr><td>$variant</td><td>$(basename "$image_file")</td><td>$image_size</td><td>$compressed_size</td></tr>"
        fi
    done
...
main() {
    ...
    echo "🚀 VexFS CI/CD Build Pipeline (QEMU-based)"
    echo "=========================================="
...
case "${1:-}" in
    --help|-h)
        cat << EOF
VexFS CI/CD Build Pipeline (QEMU-based)
