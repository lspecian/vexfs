 ...
 * vector database capabilities with high performance.
 * 
 * Building on: 54,530 ops/sec basic operations (vexfs_fixed.c)
 * High performance for both basic and vector operations
 */
...
/* Note: Structures are now defined in vexfs_v2_uapi.h and vexfs_search.h */
...
/* Include FFI header for Rust integration */
#include "vexfs_ffi.h"

/* Simple file storage system for testing */
...
static LIST_HEAD(vexfs_file_list);
static DEFINE_SPINLOCK(vexfs_file_lock);

/* Find file entry by inode number */
static struct vexfs_file_entry *vexfs_find_file_entry(unsigned long inode_num)
{
    struct vexfs_file_entry *entry;
    
    spin_lock(&vexfs_file_lock);
    list_for_each_entry(entry, &vexfs_file_list, list) {
        if (entry->inode_num == inode_num) {
            spin_unlock(&vexfs_file_lock);
            return entry;
        }
    }
    spin_unlock(&vexfs_file_lock);
...
static struct vexfs_file_entry *vexfs_create_file_entry(unsigned long inode_num, const char *filename)
{
    struct vexfs_file_entry *entry;
    ...
    spin_lock(&vexfs_file_lock);
    list_add(&entry->list, &vexfs_file_list);
    spin_unlock(&vexfs_file_lock);
    
    printk(KERN_INFO "VexFS: Created file entry for inode %lu with filename '%s'\n",
           inode_num, filename ? filename : "(null)");
    return entry;
}

/* Temporary stub implementations for FFI functions */
int vexfs_rust_read_file(void *inode_ptr, void *file_ptr, void *buf,
                        __u64 count, __u64 pos, __u64 *bytes_read)
{
    ...
    /* Find file entry by inode number */
    entry = vexfs_find_file_entry(inode->i_ino);
    if (!entry) {
        printk(KERN_WARNING "VexFS: File entry not found for inode %lu\n", inode->i_ino);
        *bytes_read = 0;
        return -ENOENT;
    }
    ...
    to_copy = (count < entry->size - pos) ? count : (entry->size - pos);
    
    if (copy_to_user(buf, entry->data + pos, to_copy)) {
        return -EFAULT;
    }
    
    *bytes_read = to_copy;
    printk(KERN_INFO "VexFS: Read %zu bytes from inode %lu at pos %llu\n",
           to_copy, inode->i_ino, pos);
    return 0;
}

int vexfs_rust_write_file(void *inode_ptr, void *file_ptr, const void *buf,
                         __u64 count, __u64 pos, __u64 *bytes_written)
{
    ...
    entry = vexfs_find_file_entry(inode->i_ino);
    if (!entry) {
        entry = vexfs_create_file_entry(inode->i_ino, NULL);
        if (!entry) {
            return -ENOMEM;
        }
    }
    ...
    /* Calculate new size */
    new_size = (pos + count > entry->size) ? pos + count : entry->size;
    
    /* Reallocate if needed */
    if (new_size > entry->capacity) {
        new_data = krealloc(entry->data, new_size, GFP_KERNEL);
        if (!new_data) {
            return -ENOMEM;
        }
        entry->data = new_data;
        entry->capacity = new_size;
    }
    
    /* Copy data from user space */
    if (copy_from_user(entry->data + pos, buf, count)) {
        return -EFAULT;
    }
    ...
    *bytes_written = count;
    printk(KERN_INFO "VexFS: Wrote %llu bytes to inode %lu at pos %llu (new size: %zu)\n",
           count, inode->i_ino, pos, entry->size);
    return 0;
}
                                  ...
                                  size_t count, loff_t *ppos)
{
    ...
    /* Call into Rust FFI for actual data retrieval */
    ret = vexfs_rust_read_file(inode, file, buf, count, *ppos, &bytes_read);
    if (ret != 0) {
        printk(KERN_ERR "VexFS: Failed to read file data: %d\n", ret);
        return ret;
    }
                                   ...
                                   size_t count, loff_t *ppos)
{
    ...
    /* Call into Rust FFI for actual data storage */
    ret = vexfs_rust_write_file(inode, file, buf, count, *ppos, &bytes_written);
    if (ret != 0) {
        printk(KERN_ERR "VexFS: Failed to write file data: %d\n", ret);
        return ret;
    }
    
    /* Update file size and position */
    if (*ppos + bytes_written > inode->i_size) {
        inode->i_size = *ppos + bytes_written;
...
/* Custom getattr implementation that reports correct file sizes */
static int vexfs_v2_getattr(struct mnt_idmap *idmap,
                             ...
                             u32 request_mask, unsigned int flags)
{
    ...
    /* First call the generic getattr to fill basic fields */
    ret = simple_getattr(idmap, path, stat, request_mask, flags);
    if (ret)
        return ret;
    
    /* Update file size from our storage system */
    entry = vexfs_find_file_entry(inode->i_ino);
    if (entry) {
        /* Update both the stat structure and inode size */
        stat->size = entry->size;
        inode->i_size = entry->size;
        printk(KERN_DEBUG "VexFS: getattr for inode %lu, size %zu\n",
               inode->i_ino, entry->size);
    } else {
        /* No file entry found, report size as 0 */
        stat->size = 0;
        inode->i_size = 0;
        printk(KERN_DEBUG "VexFS: getattr for inode %lu, no entry found, size 0\n",
               inode->i_ino);
    }
...
static int vexfs_v2_readdir(struct file *file, struct dir_context *ctx)
{
    ...
    /* Emit files from our storage system */
    spin_lock(&vexfs_file_lock);
    list_for_each_entry(entry, &vexfs_file_list, list) {
        if (pos++ < ctx->pos - 2)
            continue;
            
        if (!dir_emit(ctx, entry->filename ? entry->filename : "unknown",
                     entry->filename ? strlen(entry->filename) : 7,
                     entry->inode_num, DT_REG)) {
            spin_unlock(&vexfs_file_lock);
            return 0;
        }
        ctx->pos++;
                                      ...
                                      unsigned int flags)
{
    ...
    /* Look for the file in our storage system */
    spin_lock(&vexfs_file_lock);
    list_for_each_entry(entry, &vexfs_file_list, list) {
        /* Match the requested filename with stored filename */
        if (entry->filename && strcmp(entry->filename, dentry->d_name.name) == 0) {
            inode = iget_locked(dir->i_sb, entry->inode_num);
            if (inode) {
                           ...
                           struct dentry *dentry, umode_t mode, bool excl)
{
    ...
    /* Create file entry in storage system */
    if (!vexfs_create_file_entry(inode->i_ino, dentry->d_name.name)) {
        printk(KERN_WARNING "VexFS: Failed to create file entry for inode %lu\n", inode->i_ino);
        /* Continue anyway - the inode is created, storage entry can be created on first write */
    }
    
    d_instantiate(dentry, inode);
    printk(KERN_INFO "VexFS: Created file '%s' with inode %lu\n",
