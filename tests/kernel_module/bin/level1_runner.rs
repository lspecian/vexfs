    ...
    while i < args.len() {
        match args[i].as_str() {
            ...
            "--timeout" => {
                ...
                let timeout_secs: u64 = args[i + 1].parse().unwrap_or_else(|_| {
...
fn parse_build_variant(variant: &str) -> BuildVariant {
    match variant.to_lowercase().as_str() {
        "safe" => BuildVariant::Safe,
        "safe-ffi" => BuildVariant::SafeFfi,
        "c-only" => BuildVariant::COnly,
...
fn print_usage(program_name: &str) {
    ...
    println!("    --timeout <SECONDS>          Maximum time to wait for operations");
    println!("                                 Default: 30");
    ...
    println!("      safe     - Safe build without FFI (safest for host testing)");
    println!("      safe-ffi - Safe FFI build with error handling");
    println!("      c-only   - C-only build without Rust components");
    println!("      standard - Standard build with full FFI (use with caution)");
    println!();
    println!("EXAMPLES:");
    println!("    # Run safe tests without sudo");
    ...
    println!();
    println!("    # Test specific build variant with custom paths");
    println!("    {} --build-variant safe-ffi \\", program_name);
...
mod tests {
    ...
    fn test_parse_build_variant() {
        assert_eq!(parse_build_variant("safe"), BuildVariant::Safe);
        assert_eq!(parse_build_variant("SAFE"), BuildVariant::Safe);
        assert_eq!(parse_build_variant("safe-ffi"), BuildVariant::SafeFfi);
        assert_eq!(parse_build_variant("c-only"), BuildVariant::COnly);
