...
// Note: ioctl_none is not available in nix 0.27+, using alternative approach
use nix::libc;
...
// Define IOCTL operations using direct libc calls
// Note: Simplified implementation for compatibility
pub fn vexfs_get_status(fd: std::os::unix::io::RawFd) -> nix::Result<i32> {
    use nix::errno::Errno;
    let request = (VEXFS_IOCTL_MAGIC as u32) << 8 | (VEXFS_IOCTL_GET_STATUS as u32);
    unsafe {
        let result = libc::ioctl(fd, request as libc::c_ulong);
        if result == -1 {
            Err(Errno::last())
        } else {
            Ok(result as i32)
